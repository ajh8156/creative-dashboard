...
python marketing/scripts/pipeline/preprocess-airbridge.py --type creative --date $date

Write-Host "--- 2. [Step 2] 소재 리포트 및 컨텍스트 생성 시작 ---" -ForegroundColor Cyan
python marketing/scripts/pipeline/generate-creative-report.py --date $date

Write-Host "--- 3. [Step 3] 대시보드 데이터 가공 시작 (Parquet) ---" -ForegroundColor Cyan
python marketing/scripts/pipeline/update_dashboard_data.py
...
Write-Host "리포트: marketing/reports/creative-performance-$date.md"
